...
public:
	...
	void readAndWrite(int status)
	{
		...
		while (input.tellg() < fileSize)
		{
			...
			if (ok == 0) //if record does not meet update conidition, write it to temp.bin, else update it and then write to temp.bin
			{
			...
			else if (ok == 1) //if record does not meet delete conidition, write it to temp.bin, else ignore it (so it gets deleted)
			{
				for (int i = 0; i < noOfColumns; i++)
				{
					...
					if ((*this->col[i]).getValue() != nullptr)
					{
						if (strcmp((*this->col[i]).getName(), this->storage.c_str()) == 0)
						{
							char* newValue = new char[strlen(this->updatedValue.c_str()) + 1];
							strcpy(newValue, this->updatedValue.c_str());
							newValue[strlen(this->updatedValue.c_str())] = '\0';
							(*this->col[i]).setValue(newValue);
						}
						cout << (*this->col[i]).getValue();
						var = strlen((*this->col[i]).getValue());
						tempFile.write((char*)&var, sizeof(int));
					...
					else
					{
						if (strcmp((*this->col[i]).getName(), this->storage.c_str()) == 0)
						{
							(*this->col[i]).setInteger(atoi(this->updatedValue.c_str()));
						}
						cout << (*this->col[i]).getInteger();
						var = (*this->col[i]).getInteger();
						tempFile.write((char*)&var, sizeof(int));
		...
		this->tableName += ".bin";
		std::rename("temp.bin", (this->tableName).c_str());
		cout << endl;

	}

	void checkCondition() //checks for keywords SET, WHERE, stores table name
	{
		int pos = 6, status;
		string condition;
		...
		if (strcmp(this->buffer, "SET") == 0)
		{
			cout << endl << "ERROR! Missing table name.";
			return;
		}
		this->tableName = this->buffer;
		cout << endl << "Updating in table " << this->buffer << endl;
		this->setBuffer(this->nextWord(pos, tempCommand));
		if (strcmp(this->buffer, "SET") == 0)
		{
			this->setBuffer(this->nextWord(pos, tempCommand));
		...
		{
			cout << endl << "Missing keyword SET" << endl;
			return;
		}
		if (this->buffer != nullptr)
		this->storage += this->buffer;
		else {
			cout << endl << "ERROR! Missing collumn name to be updated.";
			return;
		}
		this->setBuffer(this->nextWord(pos, tempCommand));
		if (this->buffer != nullptr)
		...
		if (this->buffer != nullptr)
		{
			this->updatedValue += this->buffer;
			this->setBuffer(this->nextWord(pos, tempCommand));
		}
		else {
			cout << endl << "ERROR! Missing new value for the column to be updated.";
			return;
		}
